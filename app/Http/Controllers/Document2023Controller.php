...
use App\Imports\Document2023Import;
use Maatwebsite\Excel\Facades\Excel;

class Document2023Controller extends Controller
{
    public function sewaIndex()
    {
        $title = 'Dokumen Sewa 2023';
        $documents = Document2023::with(['user_pkn', 'user_penilai'])->where('jenis_persetujuan', 'Sewa')->get();
        ...
        return view('dokumen.dokumen2023.sewa-index', compact('title', 'documents'));
    }

    public function penjualanIndex()
    {
        $title = 'Dokumen Penjualan 2023';
        $documents = Document2023::with(['user_pkn', 'user_penilai'])->where('jenis_persetujuan', 'Penjualan')->get();
        ...
        return view('dokumen.dokumen2023.penjualan-index', compact('title', 'documents'));
    }

    public function penghapusanIndex()
    {
        $title = 'Dokumen Penghapusan 2023';
        $documents = Document2023::with(['user_pkn', 'user_penilai'])->where('jenis_persetujuan', 'Penghapusan')->get();
        ...
        return view('dokumen.dokumen2023.penghapusan-index', compact('title', 'documents'));
    }
    ...
    public function create()
    {
        ...
        return view('dokumen.dokumen2023.create', compact('title', 'pkn_conceptors', 'penilai_conceptors'));
    }
    ...
    public function store(Request $request)
    {
        ...
        Document2023::create($data);
        $type = strtolower($request->jenis_persetujuan);
        $path = env('APP_URL') . "/dokumen/$type/2023";
    ...
    {
        $title = 'Edit Dokumen 2023';
        $document = Document2023::with(['user_pkn', 'user_penilai'])->findOrFail($id);
        $progress = json_decode($document->progress);
        $pkn_conceptors = User::where('role', 'Admin Pkn')->orderBy('name')->get();
        $penilai_conceptors = User::where('role', 'Admin Penilai')->orderBy('name')->get();
        return view('dokumen.dokumen2023.edit', compact('title', 'document', 'progress', 'pkn_conceptors', 'penilai_conceptors'));
    }
    ...
    public function import(Request $request)
    {
        ...
        $file->move('Document2023', $fileName);

        Excel::import(new Document2023Import, public_path('Document2023/' . $fileName));
