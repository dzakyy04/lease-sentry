...
use App\Imports\Document2022Import;
use Maatwebsite\Excel\Facades\Excel;

class Document2022Controller extends Controller
{
    public function sewaIndex()
    {
        $title = 'Dokumen Sewa 2022';
        $documents = Document2022::with(['user_pkn', 'user_penilai'])->where('jenis_persetujuan', 'Sewa')->get();
        ...
        return view('dokumen.dokumen2022.sewa-index', compact('title', 'documents'));
    }

    public function penjualanIndex()
    {
        $title = 'Dokumen Penjualan 2022';
        $documents = Document2022::with(['user_pkn', 'user_penilai'])->where('jenis_persetujuan', 'Penjualan')->get();
        ...
        return view('dokumen.dokumen2022.penjualan-index', compact('title', 'documents'));
    }

    public function penghapusanIndex()
    {
        $title = 'Dokumen Penghapusan 2022';
        $documents = Document2022::with(['user_pkn', 'user_penilai'])->where('jenis_persetujuan', 'Penghapusan')->get();
        ...
        return view('dokumen.dokumen2022.penghapusan-index', compact('title', 'documents'));
    }
    ...
    public function create()
    {
        ...
        return view('dokumen.dokumen2022.create', compact('title', 'pkn_conceptors', 'penilai_conceptors'));
    }
    ...
    public function store(Request $request)
    {
        ...
        Document2022::create($data);
        $type = strtolower($request->jenis_persetujuan);
        $path = env('APP_URL') . "/dokumen/$type/2022";
    ...
    {
        $title = 'Edit Dokumen 2022';
        $document = Document2022::with(['user_pkn', 'user_penilai'])->findOrFail($id);
        $progress = json_decode($document->progress);
        $pkn_conceptors = User::where('role', 'Admin Pkn')->orderBy('name')->get();
        $penilai_conceptors = User::where('role', 'Admin Penilai')->orderBy('name')->get();
        return view('dokumen.dokumen2022.edit', compact('title', 'document', 'progress', 'pkn_conceptors', 'penilai_conceptors'));
    }
    ...
    public function import(Request $request)
    {
        ...
        $file->move('Document2022', $fileName);

        Excel::import(new Document2022Import, public_path('Document2022/' . $fileName));
