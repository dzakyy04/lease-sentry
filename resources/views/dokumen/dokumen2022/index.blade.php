...
@push('js')
    ...
    <script>
        ...
        $(document).on('show.bs.modal', '#deleteDocument2022Modal', function(event) {
            const button = $(event.relatedTarget);
            const id = button.data('id');
            const satker = button.data('satker');
            const modal = $(this);
            const deleteForm = $('#deleteDocument2022Form');
            const deleteMessage = $('#deleteMessage');

            deleteMessage.html(`Apakah anda yakin ingin menghapus dokumen <strong>${satker}</strong>`);
            deleteForm.attr('action', '{{ route('document2022.delete', ':id') }}'.replace(':id', id));
        });
    </script>
    <script>
        $(document).ready(function() {
            $('tbody tr').on('dblclick', function() {
                ...
                window.location.href = '{{ route('document2022.edit', ':id') }}'.replace(':id', id);
            });
        });
    </script>
@endpush
...
@section('content')
        ...
        <div class="nk-block nk-block-lg">
            <div class="nk-block-head">
                <div class="nk-block-head-content">
                    <h4 class="nk-block-title">Dokumen 2022</h4>
                </div>
            </div>
            <div class="card card-bordered card-preview">
                <div class="card-inner">
                    @can(['admin-pkn-super-admin'])
                        <div class="d-flex">
                            <a href="{{ route('document2022.create') }}" class="btn btn-primary mb-2 me-2">
    ...
    <div class="modal fade" id="uploadModal" tabindex="-1" aria-labelledby="uploadModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('document2022.import') }}" class="form-validate is-alter" method="post"
    ...
    {{-- Delete Modal --}}
    <div class="modal fade" id="deleteDocument2022Modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Dokumen 2022</h5>
                    <a href="#" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <em class="icon ni ni-cross"></em>
                    </a>
                </div>
                <div class="modal-body">
                    <form action="" method="POST" class="form-validate is-alter" id="deleteDocument2022Form">
                        @csrf
                        @method('delete')
